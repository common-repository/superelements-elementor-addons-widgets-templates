...
use \SuperElement\Templates\TemplatesInit as SE_init;
...
class Load{
    ...
    public function load_template_views() {
        include SE_init::dir() . 'views/templates.php';
    }
   
    public static function get_library() {
        if ( is_null( self::$library_data ) ) {
            self::$library_data = new TemplateApi();
        }
        return self::$library_data;
    }
 
    public function register_ajax_actions( Ajax $ajax ) {
        $ajax->register_ajax_action( 'get_se_addon_library_data', function( $data ) {
